...
if [ "$TRAVIS_PULL_REQUEST" != "false" -o "$TRAVIS_REPO_SLUG" != "fossasia/susi_viberbot" -o  "$TRAVIS_BRANCH" != "$DEPLOY_BRANCH" ]; then
...
openssl aes-256-cbc -K $encrypted_e37515051e9a_key -iv $encrypted_e37515051e9a_iv -in ./kubernetes/travis/susi-telegrambot-85cf0ec296e9.json.enc -out susi-telegrambot-85cf0ec296e9.json -d
mkdir -p lib
